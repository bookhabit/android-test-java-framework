...
interface DailyStepsDao {
    ...
    @Query("SELECT SUM(steps) FROM daily_steps WHERE date LIKE :monthPrefix || '%'")
    ...
    // 특정 날짜의 걸음수만 업데이트
    @Query("UPDATE daily_steps SET steps = :steps, timestamp = :timestamp WHERE date = :date")
    suspend fun updateStepsByDate(date: String, steps: Long, timestamp: Long = System.currentTimeMillis())
